	...
	public String validate(@QueryParam("type") String type, @QueryParam("style") String style) {
		...
		try {
			...
			for (int i = 0; i < data.size(); i++) {
				...
				validationResults += validateCounterparty(data.get(i)) + " for customer"+data.get(i).getCustomer()+" with type "+data.get(i).getType()+" for "+i+1+"\'st record\n\n";
				validationResults += validateCurrency(data.get(i)) + " for customer"+data.get(i).getCustomer()+" with type "+data.get(i).getType()+" for "+i+1+"\'st record\n\n";

				if (type.equals("spot") || type.equals("forward")) {
					validationResults += validateValueDateBeforeTradeDate(data.get(i), type) + " for customer"+data.get(i).getCustomer()+" with type "+data.get(i).getType()+" for "+i+1+"\'st record\n\n";
					validationResults += validateValueDateNonWorkingDay(data.get(i), type) + " for customer"+data.get(i).getCustomer()+" with type "+data.get(i).getType()+" for "+i+1+"\'st record\n\n";
				}

				if (type.equals("options")) {
					if (style.equals("AMERICAN")) {
						validationResults += validateExcerciseDate(data.get(i)) + " for customer"+((FXOptions) data.get(i)).getCustomer()+" with type "+((FXOptions) data.get(i)).getCustomer()+" and style"+((FXOptions) data.get(i)).getStyle()+" for "+i+1+"\'st record\n\n";
					}
					validationResults += validateExpiryDate(data.get(i),style) + " for customer"+((FXOptions) data.get(i)).getCustomer()+" with type "+((FXOptions) data.get(i)).getCustomer()+" and style"+((FXOptions) data.get(i)).getStyle()+" for "+i+1+"\'st record\n\n";
				}
			}
